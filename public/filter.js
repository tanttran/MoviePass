...
clickGenre.addEventListener('change', function(e) {
  e.preventDefault();
  var xhr = new XMLHttpRequest();
  xhr.onload = function () {
    if(xhr.status ===200) {
  ...
  // var genreValue;
  // for (k=1; k<20; k++) {
  //   var genreText = document.getElementById('genreInput').value;
  //   if(genreText == 'Drama') {
  //     genreValue = 18 +',';
  //   } else if (genreText == 'Adventure') {
  //     genreValue = 12 +',';
  //   }

  // }
  var genreValue = clickGenre.options[clickGenre.selectedIndex].value;
  xhr.open('POST', 'http://localhost:1337/movie/filterGenre', true);
  xhr.send(genreValue);
}, false);

clickKeyword.addEventListener('click', function(e) {
  e.preventDefault();
  var xhr = new XMLHttpRequest();
  xhr.onload = function () {
    if(xhr.status ===200) {
